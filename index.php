...
$sql = "DELETE FROM offers WHERE id='$theID'";
...
<html lang="en" class="h-full">
   <head>
      ...
      <title>Offers - <?php echo $siteName ?></title>
      <!-- Meta Tags -->
               ...
               <header class="flex-shrink-0">
                  <div class="mx-auto max-w-7xl md:flex md:items-center md:justify-between">
                     <div class="min-w-0 flex-1">
                        <h1 class="sm:pl-8 pl-5 pb-4 border-b text-xl font-bold leading-tight tracking-tight text-gray-800 sm:truncate">Offers</h1>
                     </div>
                  </div>
               </header>
               <main class="flex-1">
                  <div class="min-h-full mx-auto max-w-7xl py-8 sm:px-6 lg:px-8">
                      <div id="projects" class="grid grid-cols-3 gap-4">
                          <a href="/new/offer">
   <li class="xl:h-[220px] h-[300px] justify-center border-2 border-dashed rounded-md shadow-sm bg-white relative flex gap-x-6 px-10 xl:py-20 py-32 hover:bg-gray-50 sm:px-6 lg:px-6">
   <div class="min-w-0">
      <img class="ml-auto mr-auto w-[25px]" src="/assets/images/plus.svg">
      <p class="mt-1.5 text-base font-semibold leading-6 text-gray-600">New offer</p>
   </div>
</li>
</a>
<?php
...
$theid = $id;
$sql = "SELECT * FROM offers WHERE user='$id' ORDER BY `id` DESC";
...
?>
    <div class="xl:h-[220px] h-[300px] pb-4 ring-1 ring-gray-900/5 rounded-md shadow-sm bg-white relative gap-x-6 hover:bg-gray-50">
   <div class="pb-3 pt-4 border-b items-center gap-3 justify-between flex">
   <div class="items-center gap-3 flex">
       <div class="pl-4">
       <i style="<?php if($row['offer_type'] == "product") { } else { echo "display: none;"; } ?>" class="rounded-lg w-[50px] h-[50px] p-3 text-white bg-green-400" data-feather="gift"></i>
       <i style="<?php if($row['offer_type'] == "service") { } else { echo "display: none;"; } ?>" class="rounded-lg w-[50px] h-[50px] p-3 text-white bg-blue-500" data-feather="smile"></i>
       <i style="<?php if($row['offer_type'] == "job") { } else { echo "display: none;"; } ?>" class="rounded-lg w-[50px] h-[50px] p-3 text-white bg-red-500" data-feather="briefcase"></i>
       </div>
       <div class="block">
          <div class="font-bold"><?php echo $row['name'] ?></div>
          <div class="text-gray-400"><?php echo mb_strimwidth($row['description'], 0, 32, '...'); ?></div>
       </div>
  </div>
       <form id="delete-<?php echo $row['id'] ?>" action="/" method="POST">
...
<div id="dropdown-<?php echo $row['id'] ?>" class="mL border z-10 hidden bg-white divide-y divide-gray-100 rounded-md w-24">
   ...
   <div class="pl-4 grid grid-cols-1 grid xl:grid-cols-2 gap-4 mt-4 min-w-0">
      <div>
        ...
        <p class="capitalize text-black font-semibold"><?php echo $row['offer_type'] ?></p>
      </div>
      <div>
        <p class="text-sm text-gray-500 font-semibold">Website</p>
        <p class="text-black font-semibold"><?php echo $row['website'] ?></p>
      </div>
      <div>
        <p class="text-sm text-gray-500 font-semibold">Location</p>
        <p class="text-black font-semibold"><?php echo $row['location'] ?></p>
      </div>
   </div>
</div>
